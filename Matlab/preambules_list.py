...
ideal_one = amplitude*np.concatenate((np.ones(round(0.5E-6/fd_real)), np.zeros(round(0.5E-6/fd_real))))
ideal_zero = amplitude*np.concatenate((np.zeros(round(0.5E-6/fd_real)), np.ones(round(0.5E-6/fd_real)))) #100ilgis

#Regular
ideal_preambule = amplitude*np.concatenate((np.ones(round(0.5E-6/fd_real)),  np.zeros(round(0.5E-6/fd_real)), np.ones(round(0.5E-6/fd_real)), np.zeros(round(2E-6/fd_real)), np.ones(round(0.5E-6/fd_real)), np.zeros(round(0.5E-6/fd_real)), np.ones(round(0.5E-6/fd_real))))
negative_preambule = amplitude*np.concatenate((np.ones(round(0.5E-6/fd_real)),  -1*np.ones(round(0.5E-6/fd_real)), np.ones(round(0.5E-6/fd_real)), -1*np.ones(round(2E-6/fd_real)), np.ones(round(0.5E-6/fd_real)), -1*np.ones(round(0.5E-6/fd_real)), np.ones(round(0.5E-6/fd_real))))
normalized_preambule = np.copy(negative_preambule)
normalize_coefficients(normalized_preambule)

#Extended
extended_preambule = np.concatenate((np.copy(ideal_preambule), np.zeros(round(3E-6/fd_real))))
extended_negative_preambule = np.concatenate((np.copy(negative_preambule), -1*np.ones(round(3E-6/fd_real))))
...
preambule_list = [
    ...
    PreambuleVariantDifferentiated("Differentiated", differentiated_preambule, diff_amount),
]